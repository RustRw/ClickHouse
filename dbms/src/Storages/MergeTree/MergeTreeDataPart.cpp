...
{
    const auto & columns = storage.table_declaration.getColumnsList();
...
void MergeTreeDataPart::accumulateColumnSizes(ColumnToSize & column_to_size) const
{
    ...
    for (const NameAndTypePair & name_type : storage.table_declaration.columns)
    {
...
void MergeTreeDataPart::loadColumns(bool require)
{
    ...
    if (!Poco::File(path).exists())
    {
        ...
        for (const NameAndTypePair & column : storage.table_declaration.getColumnsList())
