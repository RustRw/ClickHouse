...
void MergeTreeReader::fillMissingColumns(Block & res, const Names & ordered_names, bool always_reorder)
{
    ...
    try
    {
        ...
        for (const auto & requested_column : columns)
        {
            ...
            {
                should_sort = true;
                if (storage.table_declaration.column_defaults.count(requested_column.name) != 0)
                {
        ...
        if (should_evaluate_defaults)
            evaluateMissingDefaults(res, columns, storage.table_declaration.column_defaults, storage.context);
